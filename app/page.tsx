...
// import { prompts, tools } from "@/lib/data"; // <-- Ye line hata di humne
import { supabase } from "@/lib/supabase"; // <-- Ye naya import hai
import { useMonetag } from "@/hooks/useMonetag";
import Navbar from "@/components/Navbar";
import { Copy, Check, ExternalLink, Zap, Bot, Loader2 } from "lucide-react";

// Types define kar lete hain taaki error na aaye
interface Prompt {
  id: number;
  title: string;
  category: string;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Prompts Table se data lana
        const { data: promptsData, error: promptsError } = await supabase
          .from('prompts') // Make sure table name 'prompts' hi ho
          .select('*');

        // Tools Table se data lana
        const { data: toolsData, error: toolsError } = await supabase
          .from('tools') // Make sure table name 'tools' hi ho
          .select('*');
